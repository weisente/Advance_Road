...
import example.weisente.top.baselibrary.recycleviewbase.adapter.OnLongClickListener;

/**
 * Created by san on 2017/11/1.
 *Adapter 主要就三个方法
 *getItemCount      这里是 mHeaderViews + mFooterViews + 通用view的数量
 *getItemViewType   根据不同的位置 返回不同的数值（类型）
 * onBindViewHolder 根据位置绑定相对应的viewholder
 *
 */
